...
const getR6STournaments = async function (req, res) {
  let finalResponse = {};
  try {
    console.log("getting tournaments");
    const recentBody = await fetch(
      `${rootUrl}r6siege/tournaments/past?search[tier]=s&page=1&per_page=3&sort=-begin_at`,
      options
    );
    const recentData = await recentBody.json();
    finalResponse.recent = recentData;
    const currentBody = await fetch(
      `${rootUrl}r6siege/tournaments/running?search[tier]=s&sort=&page=1&per_page=3`,
      options
    );
    const currentData = await currentBody.json();
    finalResponse.current = currentData;
    const upcomingBody = await fetch(
      `${rootUrl}r6siege/tournaments/upcoming?search[tier]=s&sort=&page=1&per_page=3`,
      options
    );
